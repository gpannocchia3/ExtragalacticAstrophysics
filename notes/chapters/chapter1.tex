...
\end{tcolorbox}
\par The validity of such principles on the largest scales is manifested in a number of different observations, such as number counts of galaxies and observations of diffuse X-ray and $\gamma$-ray backgrounds, although it is most clear in the cosmic microwave background (CMB). We're going to spend quite some time with this last lad.
\begin{figure}[h!]
    \centering 
...
\begin{equation}
    ...
    \label{eq:FRW}
\end{equation}
\par The coordinates used in (\ref{eq:FRW}), in which the metric is free of cross-terms $\text{d}t\text{d}x^{i}$ and the coefficient $\text{d}t^{2}$ is independent of $x^{i}$, are known as \emph{comoving coordinates} and are dimensionless. Comoving coordinates \emph{do not change at any time}. We can use an alternative form of the metric, obtained by introducing a new radial coordinate $\chi$ defined by
\begin{equation}
...
\end{equation}
The spacelike section of the FLRW metric consists of the spatial metric for a uniformly curved space of radius $R_{0}$ (the present-day radius of curvature) scaled by the square of the scale factor $a(t)$.
...
\end{equation}
and the factor $S_{k}(r)$ is simply (\ref{eq:sphere_radius}) scaled with the present-day radius of curvature $R_{0}$. Sometimes it is useful to perform a slight manipulation of the FLRW metric (\ref{eq:FRW}) and define a conformal time 
\begin{equation*}
...
In our description of the Universe we'd like to observationally determine a number of quantities to try to infer which of the models proposed by the FLRW metric\footnote{As per any GR subject in this notes, I'll follow \cite{carroll}. You can find this discussion at §8.5.}
\begin{equation*}
...
Ignoring the angular dependence, i.e. a point object for which $\text{d}\Omega = 0$, the proper distance between the observer at the origin $r = 0$ and a galaxy at $(r,\theta,\phi)$ can be found using the FLRW metric\footnote{Please note that we're using the metric in the form (\ref{eq:FRW_revisited}) where weìve implicitly defined $\text{d}r = R_{0}\text{d}\chi$. It's either that or we're implicitly assuming the Universe to be flat.} at a fixed time $t$
\begin{equation*}
...
\end{equation}
so that if $q_{0} > 0$, then $\ddot{a} < 0$ and the expansion of the Universe decelerates. Conversely, if $q_{0} < 0$, then $\ddot{a} > 0$ and the expansion of the Universe accelerates. Recent measurements of $q_{0}$ \cite{Riess_2022} have set
\begin{equation}
    q_{0} = -0.510 \pm 0.024
...
\par Using (\ref{eq:a_expansion}) in the proper distance definition (for a radial null geodesic), we can calculate
\begin{equation}
    d_{p}(t_{0}) = a(t_{0})\int_{t_{e}}^{t_{0}} \frac{c\,\text{d}t}{a(t)} \approx c(t_{0}-t_{e}) + \frac{cH_{0}}{2}(t_{0}-t_{e})^{2}
\end{equation}
...
\par We can set the comoving radial distance to $r_{e}$, so that photons emitted by an object at this distance will pass (today) through a sphere of proper radius $R = a(t_{0})r_{e} = r_{e}$. The area of such sphere at fixed time can be derived from the coefficient of $\text{d}\Omega$ in (\ref{eq:FRW_revisited}), yielding
\begin{equation*}
...
\end{figure}
\par The luminosity distance is something we might hope to measure, since there are some astrophysical sources whose absolute luminosity are known (see Chapter \ref{ch:ii}), but $\chi$ (and, for extension, the comoving distance $r_{e}$), is not observable, so we have to remove it from our equation. On a null radial geodesic (for convenience), we have
\begin{equation*}
...
\end{equation*}
where we've defined $E(z) = H(z)/H_{0}$. Note that $R_{0}$ drops out when $k = 0$, which is good, because in that case (flat Universe) the radius of curvature is a completely arbitrary parameter. Still, it is more common to speak in terms of $\Omega_{c,0} = -k/R_{0}^{2}H_{0}^{2}$, which can be measured. In terms of this parameter we have
\begin{equation*}
...
\par Let us now consider a simple case for which we can find a solution right away. If the Universe is roughly flat, $S_{k}(r_{e}) \approx r_{e} \approx d_{p}(t_{0})$, and plugging in the second order expansion for the proper distance (\ref{eq:properdistance_rs_second}) into (\ref{eq:luminositydistance}) we find
\begin{equation}
...
\end{equation}
to derive the so called \emph{Friedmann's equations}. We will choose to model matter and energy by a perfect fluid. This means that if we consider a fluid at rest in the comoving coordinates, then $u^{\mu} = (1,0,0,0)$ and the energy momentum tensor associated with the fluid will be
\begin{equation}
    T_{\mu\nu} = (p+\rho)u_{\mu}u_{\nu} +pg_{\mu\nu} \quad T = -\rho +3p
...
\begin{equation}
    0 = \nabla_{\mu}T^{\mu}_{\; \;0} = -\partial_{0}\rho-3\frac{\dot{a}}{a}(\rho+p)
    \label{eq:fluidequation}
\end{equation}
which is sometimes known as \emph{fluid equation}. Here $\rho$ is the energy density and $p$ is the pressure. Please note that we're working in units $c=1$.
\par Making use of the expressions you can find in Appendix C, we can solve explicitly (\ref{eq:einstein_eq}) and find
\begin{equation}
    \boxed{
...
\end{equation}
The cross-section for Thomson scattering isn't particularly large ($c=1$)
\begin{equation}
    \sigma_{T} = \frac{8\pi}{3}\left(\frac{e^{2}}{m_{e}}\right)^{2} = 6.6\cdot 10^{-25}\,\text{cm}^{-2} = 0.66\unit{b}
    \label{eq:thomson_ct}
\end{equation}
...
\end{equation}
where $g_{i}$ is the statistical weight of particle $i$. This expression applies in the non-relativistic regime, i.e. when $m_{i}c^{2} \gg kT$, and thus the particle had highly nonrelativistic thermal speeds.
\par At recombination the temperature is about $T \sim 3000\,\text{K}$, so $kT \sim 0.26\,\text{eV}$, while $m_{e} = 511\,\text{keV}$ and $m_{p} = 938\,\text{MeV}$. That means that electrons, protons and Hydrogen atoms are in the non-relativistic regime.
\par If we recall Saha's equation
\begin{equation}
...
\begin{equation*}
    \frac{n_{\text{H}}}{n_{e}n_{p}} = \left(\frac{m_{e}kT}{2\pi\hbar^{2}}\right)^{-3/2}\exp\left(\frac{Q}{kT}\right)
\end{equation*}
\par We can make the useful sostitution $Xn_{\text{H}} = (1-X)n_{p}$ and exploit the global neutrality of the Universe $n_{p} = n_{e}$
\begin{equation*}
    \frac{1-X}{X} = n_{p}\left(\frac{m_{e}kT}{2\pi\hbar^{2}}\right)^{-3/2}\exp\left(\frac{Q}{kT}\right)
\end{equation*}
...
\begin{equation*}
    \frac{1-X}{X} = \eta n_{\gamma}X\left(\frac{m_{e}kT}{2\pi\hbar^{2}}\right)^{-3/2}\exp\left(\frac{Q}{kT}\right)
\end{equation*}
...
\par The rate of photon scattering, when the Hydrogen is partially ionized, is
\begin{equation*}
...
\par Until now we have assumed an homogeneous and isotropic Universe. The scale factor, the Friedmann equations, and the energy densities depend only on time. But this is just an approximation, the observed Universe is never perfectly homogeneous and isotropic!
\begin{figure}[h!]
    \centering 
...
\end{equation}
through which we can express the linearized Euler's equation in a more suggestive form
\begin{equation}
...
\end{equation*}
We look for solutions with time dependence $\Delta(t)\propto e^{-i\omega t}$, so that $\ddot{\Delta} = -\omega^{2}\Delta$, hence the following dispersion relation
\begin{equation}
...
\begin{equation*}
    c_{s} = c\left(\der{p}{\rho}\right)^{1/2} = \frac{c}{\sqrt{3}}
\end{equation*}
...
\end{equation*}
where $T$ is the temperature of the gas and $\mu$ is the mean mass per gas particle. Thus, the pressure of an ideal gas will never totally vanish, but will only approach zero in the limit that the temperature approaches absolute zero. The speed of sound will be now
\begin{equation*}
...
\par When exactly did DM start to collapse? When radiation dominates even DM can’t collapse. Why? In the following we'll try to give an answer to these questions. Consider 
\begin{equation}
...
\begin{equation*}
    \ddot{\Delta}_{m}+2H\dot{\Delta}_{m} \approx 0 \to  \ddot{\Delta}_{m}+\frac{1}{t}\dot{\Delta}_{m} \approx 0
\end{equation*}
which is solved by
\begin{equation}
...
\par Recall the blackbody energy distribution (\ref{eq:blackbody_endens}), and suppose you have such a blackbody spectrum at some time $t = t_{1}$. Then, if no photons get destroyed or produced, at the later time $t_{0}$, each photon will appear redshifted according to (\ref{eq:cosmological_redshift}): $\nu_{1} = (1+z)\nu_{0}$, and the distribution will be
\begin{equation}
...
\begin{equation}
    C_{l} = \frac{1}{2l+1}\sum_{m = -l}^{l}|a_{lm}|^{2}
...
\begin{equation*}
    \rho_{\text{DM}}(t_{ls}) > \rho_{\gamma}(t_{ls}) > \rho_{\text{Baryon}}(t_{ls})
\end{equation*}
At the last scattering, the general equation for a single fluid was (\ref{eq:comovingcontrast_ft}), which is easy to generalize to the presence of two fluids (e.g., Baryons and Dark Matter)\footnote{Note that $\rho_{0}$ is not the present day matter density, but rather the unperturbed background matter density.}
\begin{equation*}
...
and if we differentiate the Baryons-to-photon ratio under the approximation of adiabatic perturbations (i.e., no spatial fluctuations in $\eta$), we find
\begin{equation*}
...
\begin{equation*}
    c_{s}^{2} = c^{2}\left(\frac{\delta p}{\delta \rho}\right)_{S}
\end{equation*}
but 
\begin{equation*}
...
\begin{enumerate}
    \item Are shifted to larger $k$ ($t_{\text{dec}}$ is fixed, peaks are at $t_{\text{dec}}kc_{s} = n\pi$);
...
\end{figure}
\begin{enumerate}
    \item Once a given scale enters the horizon, fluctuations on this scale begin to oscillate like acoustic waves (sound). The first peak corresponds to fluctuations which had time to make exactly 1 contraction since horizon entry until decoupling;
    \item The second peak corresponds to fluctuations which had time to make exactly 1 contraction and 1 expansion since horizon entry until decoupling (under density). These fluctuations entered the horizon earlier;
    \item The third peak corresponds to fluctuations which had time to make exactly 2 contractions (and 1 expansion) since horizon entry until decoupling. These fluctuations entered the horizon even earlier.
\end{enumerate}
...
\begin{tcolorbox}[enhanced,attach boxed title to top center={yshift=-3mm,yshifttext=-1mm},
  ...
  boxed title style={size=small,colframe=blue!50!black}]
  Suppose that the density of the non-baryonic dark matter at the time of last scattering was not perfectly homogeneous, but varied as a function of position. Then we could write the energy density of the dark matter as $\rho(\vec{r}) = \bar{\rho}+\delta\rho(\vec{r})$.
...
\par The temperature distortions tell us that the Universe was not perfectly homogeneous at the time of last scattering ($z\approx1090$). The angular size of the temperature fluctuations reflects in part the physical size of the density and velocity fluctuations at $z \approx 1090$.
...
\end{equation*}
In the Benchmark Model, the current horizon distance is $d_{\text{Hor}}(t_{0}) \sim 14\,\text{Gpc}$, so the angular-diameter distance to the surface of last scattering is $d_{A} \approx 12.8\,\text{Mpc}$.
...
\par If we consider a photon emitted by a galaxy at time $t_{e}$, for a light path $\text{d}s=0$, and the comoving distance traveled at time $t$
\begin{equation*}
    r = \int_{t_{e}}^{t}\frac{c\,\text{d}t'}{a(t')}
\end{equation*}
If the photon was emitted at $t_{e} = 0$, the proper (physical) distance travelled will be 
\begin{equation*}
...
\end{equation*}
and corresponds to a multipole $l_{\text{Hor}}\approx 160$. This means that only regions from slightly before the major peak in Fig.\ref{fgr:cmbtempcont} are in causal connection.
\begin{figure}[h!]
    \centering 
...
\begin{equation*}
    \frac{\dot{a}^{2}(t_{0})}{a^{2}(t_{0})} = H_{0}^{2} = \frac{8\pi G \rho_{\Lambda}}{3c^{2}} \implies \dot{a}^{2}(t) = H_{0}^{2}a^{2}(t)
\end{equation*}
which is solved by 
\begin{equation}
...
\begin{equation*}
    \frac{H^{2}(t)}{H_{0}^{2}} = \frac{\Omega_{r,0}}{a^{4}}+\frac{\Omega_{m,0}}{a^{3}}+\frac{1-\Omega_{0}}{a^{2}}
\end{equation*}
...
\end{equation*}
During the (alleged) exponential expansion of the inflationary epoch $a(t)\propto \exp(H_{i}t)$ and $H(t) = H_{i} = \text{const.}$, therefore
\begin{equation*}
...
\begin{equation*}
    ...
    \begin{array}{ll}
        ...
        a_{i}\exp(H_{i}(t_{f}-t_{i}))\left(\frac{t}{t_{f}}\right)^{1/2} & \iff t>t_{f}
    \end{array}
    \right.
\end{equation*}
...
\end{equation*}
But since $|1-\Omega(t_{f})|\sim e^{-2N}$, we can equate the two conditions to find the minimum number $N$ of $e$-foldings 
\begin{equation*}
...
\begin{align*}
    ...
    &\sim cH^{-1}
\end{align*}
with $H^{-1}\propto a^{3/2}$ for matter, and $H^{-1}\propto a^{2}$ for radiation. In this case, particle horizon is of the same order of Hubble radius. The problem in Big Bang cosmology (without inflation) is that the particle horizon $d_{\text{causal}}\sim H^{-1}$ increases faster than $\lambda \propto a$.
...
\end{equation*}
During the RDU and MDU epochs $R(a)\to 0$ as $a\to 0$. Pick any physical region whose size scales with the expansion ($\lambda \sim a$). In RDU/MDU, as $a\to 0$, the particle horizon shrinks faster than that region, so $d_{\text{causal}}/\lambda \to 0$: The regions contain many causally disconnected parts.
...
\begin{align*}
    ...
    &\frac{\text{d}}{\text{d}t}(\dot{a})^{-1} = -\frac{\ddot{a}}{(\dot{a})^{2}}<0 \implies \ddot{a} > 0 \quad (\text{requires}\; w<-1/3)
\end{align*}
\begin{figure}[h!]
    \centering
...
\par If we consider the contribution to this from two small volumes $\text{d}V_{1}$ around position $\vec{x}$ and $\text{d}V_{2}$ around position $\vec{x}+\vec{r}$ for a uniform distribution of galaxies (average density $\bar{n}$), the probability to pick a galaxy in $\text{d}V_{1}$ and a galaxy in $\text{d}V_{2}$, is $\text{d}P = \bar{n}^{2}\text{d}V_{1}\text{d}V_{2}$.
\par If the distribution is \emph{not} uniform we take the spatial average over all points separated by $\vec{r}$
\begin{equation*}
...
\end{equation*}
Skipping all the various intermediate algebric passages because they're a hassle, we're left with 
\begin{equation*}
...
\par Assume we have two sets of points. One is the sample one, designated by the letter “D” (data). It contains $N_{D}$ points. In addition there is the Poisson point set “R”, with $N_{R}$ points randomly generated. Position yourself on each data point and count the number of data points you find in a spherical shell with radius $[r, r + \Delta r]$. The total sum of points counted is designated as $DD(r)$. One may also count, for the same number of points, the number of Poisson points in the same shells, $DR(r)$.
Hence
\begin{equation*}
...
\par The product $P(k)/(2\pi)^{2}$ gives the power (i.e. the contribution to the variance of the field) per unit $k$-space volume due to the modes with wave vector $k$. In three-dimensional space, however, a thin spherical shell in $k$-space contains many different modes and has a $k$-space volume $4\pi k^{2}\text{d}k$.
...
\begin{equation*}
    \sigma \propto M^{-1/2}
\end{equation*}
The Harrison-Zeldovich spectrum, with $n=1$, will thus produce more power on small length scales than a Poisson distribution of points.
...
\par Prior to the time $t_{h}$, relativistic dark matter particles move freely in random directions with a speed close to that of light.
\par The net effect of free streaming is to wipe out any density fluctuations whose wavelength is smaller than the Hubble radius ($\sim ct_{h}$) at $t_{h}$.
...
\begin{enumerate}
    \item The first structures to collapse are regions of mass $M\sim M_{\text{min}}$ with nearly uniform overdensity. These regions collapse to form superclusters; smaller structures, such as clusters and galaxies, then form by fragmentation of superclusters. This scenario, in which the largest observable structures form first, is called the top-down scenario.
...
\end{enumerate}
Some observational facts: We're able to observe galaxies and QSOs (Quasi-Stellar Objects, a.k.a. Quasars) at $z>14$, so that small-scale structure is already present at times when the Universe had less than 2\% of its current age. On top of that, the correlation function of galaxies, both in the local Universe and at higher redshift, suggests a lot of structure on small scale. Thus, most of the dark matter in the Universe must be cold dark matter, for which free streaming has been negligible.
\begin{figure}[h!]
    \centering 
...
\par To calculate the evolution of perturbations larger than the horizon, we need GR. However, we can take a shortcut that gives the correct answer. Each region expands at a rate that depends on its density: Denser regions expand a bit more slowly, while sparser regions expand a bit more rapidly, almost as if the denser/sparser regions act like miniature closed/open Universes, each with their own expansion history.
\par As time passes, the denser regions "lag" further behind the sparser regions, and the density contrast steadily increases.
\par If a fluctuation has a scale larger than the horizon at that time (i.e. it is outside the horizon), it does not care about microphysical processes like fluid pressure, but it simply evolves according to General Relativity (Friedman equations).
\par Assuming $\Omega_{\text{tot}} = 1$ (flat Universe)
\begin{equation*}
...
\end{equation*}
This implies that super-horizon fluctuations will always be growing:
\begin{enumerate}
...
\par Combining what we've just found, we now learn that the power spectrum is modified by 
\begin{equation*}
    T^{2}(k) = 
    \left\{
    \begin{array}{ll}
...
\begin{enumerate}
    \item Cosa implica un Universo isotropo, omogeneo e in espansione? Cos’è il redshift cosmologico? Spiegare come si arriva ad una relazione tra fattore di scala e redshift.
    \item Definire la distanza propria, di luminosità e di diametro in un Universo curvo e in espansione. Come possono essere utilizzate?
    \item Discutere l’equazione di Friedmann e l’equazione fluida. Discutere come varia la densità di energia per le componenti più importanti (materia, radiazione, energia oscura) con il parametro $a$ e con il tempo.
    \item Come determineresti l’età dell’Universo utilizzando strumenti cosmologici?
    \item Discutere l’origine della radiazione cosmica di fondo in termini di ricombinazione, disaccoppiamento tra materia e radiazione e superficie di ultimo scattering.
    \item Perché un contrasto nella densità di barioni non poteva crescere prima del disaccoppiamento? Discutere in termini di massa di Jeans. E un eccesso di materia oscura come si comporta?
    \item Si consideri un singolo fluido cosmologico. Discutere equazioni (differenziali) e soluzioni per il contrasto di densità di tale fluido nel caso di un Universo statico e per uno in espansione, nel caso di materia oscura e nel caso di barioni.
    \item Discutere le fluttuazioni di temperatura nella CMB dal punto di vista fisico e statistico, introducendo il significato dello spettro di potenza angolare e della funzione di correlazione a due punti.
    \item Considerare un modello cosmologico a due fluidi, costituito da barioni e materia oscura. Discutere come i picchi dello spettro di potenza della CMB possano essere interpretati in termini di oscillazioni armoniche forzate del fluido barionico-fotonico nel potenziale gravitazionale della materia oscura.
    \item Discutere la crescita delle perturbazioni di densità dalla CMB a oggi nel regime lineare. Spiegare come il contrasto di densità barionico atteso in assenza di materia oscura si confronti con quello realmente osservato, e discutere il ruolo della materia oscura nella formazione delle strutture.
    \item Che parametri cosmologici possiamo vincolare dalla posizione e altezza dei picchi dello spettro di potenza della CMB? Discutere la forma e l’origine fisica dello spettro di potenza della CMB alle varie scale angolari.
    \item  Discutere il ruolo dell’orizzonte sonoro nello spettro di potenza della CMB. Spiegare come evolve la temperatura della radiazione di fondo con il fattore di scala e come evolve la velocità peculiare delle galassie, motivando le relazioni ottenute.
    \item Cosa sono le BAO (baryonic acoustic oscillations) e come possono essere utilizzate per testare il modello cosmologico di benchmark? Confrontare le BAO con altri metodi utili a stimare i parametri cosmologici.
    \item Si discuta come un periodo inflazionario risolva i problemi di orizzonte e di piattezza dell’Universo.
    \item  Spiegare l’uso della funzione di correlazione a due punti nello studio della distribuzione spaziale delle galassie e i principali risultati osservativi ottenuti. Assumendo uno spettro di potenza di tipo Harrison-Zel’dovich, calcolare la varianza del campo smussato su scala $R$ e discuterne le implicazioni.
    \item Perché all’equivalenza radiazione materia lo spettro di potenza della materia ha un massimo a scale intermedie?
    \item Analizzare come la predominanza di materia oscura calda o fredda possa influenzare l'evoluzione cosmologica, giustificando l’analisi con argomentazioni sia qualitative che quantitative.
    \item Cosa si intende per free streaming? Si colleghi la massa di ipotetiche particelle di materia oscura con il livello di clustering nell’Universo.
    \item Discutere la crescita di contrasto per la materia oscura, sub e super orizzonte, prima e dopo l’equivalenza radiazione materia. Come viene modificato lo spettro di potenza quando le strutture rientrano nell’orizzonte prima dell’equivalenza RM?
\end{enumerate}