...
\begin{itemize}
    ...
    \item \emph{Advanced Secondary Indicators}\footnote{I just invented this name.} involve the calibration of second level non-geometrical methods that work up to longer distances, such as the calibration of the lightcurve of Supernovae type Ia.
\end{itemize}
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{img/distanceladder.png}
    \caption{Artistical representation of the measuring procedure we've just introduced. Credits: \cite{Riess_2022}, probably.}
\end{figure}
\par Before we begin, here are some concepts that you might want to refresh.

\section*{A few reminders}

\par The luminosity of an object somewhere in the Universe is generally defined as the power emitted by said object over time (potentially over a given wavelength), and its expression is given by the relation
\begin{equation}
    L = -\frac{\text{d}E}{\text{d}t} \quad \text{erg}\cdot\text{s}^{-1}
    \label{eq:luminosity}
\end{equation}
\par As you should know, plenty of stuff in the Universe tends to emit (almost) isotropically in space. It goes without saying, however, that an observer at distance $d$ from that object won't be able to collect all the light that is emitted. What we can measure is a flux, the fraction of energy that impinges on our detector. The flux is related to the luminosity through the following relation
\begin{equation}
    F = \frac{L}{4\pi r^{2}} \quad \text{erg}\cdot\text{s}^{-1}\cdot\text{cm}^{-2}
    \label{eq:flux}
\end{equation}
...
\par Since ancient times, mankind has always tried to measure the brightness of stars (and things that were only later discovered not to be stars at all). The efforts of those that came before us are nowadays crystallized in a quantity that is still in use: The \emph{magnitude}.
...
\par Hypparchus, back in ancient Greece, classified all stars into six classes according to their apparent brightness. These classes went from class I, the brightest stars, to class VI, the less bright. According to Hypparchus, the brightest and faintest stars had an apparent brightness that is in the ratio of 100.
...
\par Similarly, we can define the \emph{absolute magnitude} as the magnitude a star would have if it were located $10\,\text{pc}$ away.
...
\begin{equation*}
    B-V = (B-V)^{0}+E[B-V]
\end{equation*}
where $E[B-V] = A_{B}-A_{V}$ is the reddening. Within our galaxy, the ratio of extinction over reddening is a well-known quantity
\begin{equation*}
...
\end{figure} 
\par Let us consider a star on the polar axis of Earth's orbit at a distance $d$ from the center of the orbit. The angle $p$ in the picture is half of the angle by which this star appears to shift with the annual motion of the Earth on the distant stars' plane.
...
\end{equation*}
where the latter approximation holds if we assume $d$ to be much larger than an astronomic unit (which for all relevant scenarios is a condition that is well satisfied).
...
\end{figure}
\par SMC and LMC galaxies\footnote{Respectively, \emph{Small Magellanic Cloud} and \emph{Large Magellanic Cloud}.} are too far for good single stars distances measurements: Most of stars have magnitudes in the G band of order $G\sim 19-20$, so that the uncertainty on the parallax measurement is, in the worst case scenario, $\sigma_{\pi}\sim 300\,\mu\text{arcsec}$. With GAIA, however, we were able to observe a large sample of stars (roughly $7\cdot10^{6}$ in the LMC and $2\cdot10^{6}$ in the SMC). All these measurements are independent, so the average parallax $\bar{\pi}$ will have an associated error of order $\sigma_{\bar{\pi}} \sim \sigma_{\pi}/\sqrt{N}$, resulting in a measure of parallax up to $0.5\%$ for the LMC and $1.5\%$ for the SMC.
...
\begin{equation*}
    a_{1}M_{1} = a_{2}M_{2} \quad a=a_{1}+a_{2}
\end{equation*}
...
\end{align*}
Recall now the definition we've given for the flux $F$ (\ref{eq:flux}), and for the surface brigtness $S$ (flux per unit solid angle for a uniform disk)
\begin{equation*}
...
Water megamasers are very luminous $22\unit{GHz}$ $\text{H}_{2}\text{O}$ spectral lines produced by compact clumps of dense, warm molecular gas. The emission is called “maser” because it is generated by microwave amplification by stimulated emission, the microwave analogue of a laser.
\par In some active galactic nuclei (AGN), these conditions are met in the central region of the galaxy, where molecular gas is concentrated in a thin circumnuclear disk around the supermassive black hole. The masing material is not a smooth continuous medium, but is thought to arise in small, discrete clumps or filaments embedded in the disk. These clumps move within the gravitational potential of the black hole, and in the best cases the disk is thin, warped, and close to Keplerian rotation.
...
\end{tcolorbox}
If we consider pulsating stars like heat engines, then radial oscillations may be the result of sound waves resonating in the stellar interior with fundamental pulsation period $\Pi$
\begin{equation*}
    \Pi = \frac{4R}{c_{s}}
\end{equation*}
...
\end{equation*}
where $\gamma $ is the ratio of specific heats for the stellar material. Assuming hydrostatic equilibrium
\begin{equation*}
...
\end{equation}
which we can integrate to obtain the crossing time 
\begin{equation}
...
\end{equation*}
meaning that the period is actually $\Pi = f(L, M, T_{\text{eff}})$. Moreover, Cepheids are intermediate mass stars burning Helium in their cores, but stellar evolution tells us that there is a mass-luminosity relation $M(L)$, so the dependency is further constrained to $\Pi(L,T_{\text{eff}})$.
\par If the layers of a star are compressed, their density and temperature will increase. Kramer’s Law, however, tells us that opacity is more sensitive to temperature than pressure, so it is very likely that the opacity of stellar material will decrease upon compression. Evidently, for stellar pulsation to occur, special conditions are required.
...
\end{equation*}
\par Upon compression, increased ionization in a partial ionization zone absorbs part of the energy input, so the temperature rises less than usual. The opacity increases and temporarily traps part of the outward radiative flux. This stored energy raises the pressure of the layer and helps drive its subsequent expansion\footnote{This is sometimes called the \emph{kappa mechanism} simply because kappa is the common symbol for opacity.}.
...
\begin{tcolorbox}[enhanced,attach boxed title to top center={yshift=-3mm,yshifttext=-1mm},
  ...
  \end{enumerate}
  During the contraction, the external regions absorb the energy released from the compression and heat up just slightly, since most of the energy is used to ionize the material. Thus, more energy is retained thanks to the increased opacity, and a pressure able to bloat up the region is produced. Similarly, the temperature decreases only a little, since most of the energy needed for the expansion is obtained through the recombination of ions and electrons from the formerly ionized nuclei; inevitably, the electron density decreases $n_{e}\downarrow$ and the opacity decreases as well $\bar{\kappa}\downarrow$. The star is thus able to cool down, contract and repeat the cycle.
\end{tcolorbox}
...
\begin{enumerate}
    \item They are young ($<100\unit{Myr}$), massive ($3-8M_{\odot}$) and bright ($M_{V}$ up to $-7$) radially pulsating variable stars;
    \item They have periods for their pulsations ranging from $\sim 1-3$ to $\sim100\unit{days}$;
...
\par Type Ia SNe stand out from the other types of SNe for the lack of Hydrogen and Helium in the spectra, the range of values of the peak luminosity, and their correlation with the light curve width.
...
\begin{equation*}
    ^{56}\text{Ni}\to^{56}\text{Co}^{56}\to\text{Fe}
\end{equation*}
...
\par For Type Ia supernovae we define the “decline rate” as the number of magnitudes the object gets dimmer in the first 15 days after maximum light in the blue magnitude band. Fast decliners are fainter objects. The relationship between the peak luminosity of a Type Ia Supernova and the speed of luminosity evolution after maximum light is called \emph{Phillip's relation} \cite{1993ApJ...413L.105P}.
\begin{figure}[h!]
    \centering 
...
\begin{itemize}
    ...
    \item Systematic effects must be controlled: Dust extinction and reddening, as well as possible evolution with redshift may reflect into changes in the progenitor population, such as age, metallicity, and the relative importance of different explosion channels.
\end{itemize}
\begin{figure}[h!]
    \centering 
...
\par The main findings of the SH0ES collaboration is summarized in Fig.\ref{fgr:shoesspecs}.
\begin{figure}[h!]
    \centering
...
where 
\begin{equation*}
    \tau(\theta,\beta) = \left[\frac{(\theta-\beta)^{2}}{2}-\psi(\theta)\right]
\end{equation*}
...
\par If the image has an angular resolution $\delta\phi$, the average number of stars in a resolution element is $\bar{N}=N(D\cdot\delta\phi)^{2}$, while the average flux from a resolution element is
\begin{equation*}
...
\end{equation*}
Because of Poisson fluctuations in the number of objects populating a given resolution element, $\bar{F}$ will not be constant, but will fluctuate from one element to another, with standard deviation
\begin{equation*}
...
\begin{equation*}
    \frac{\Delta I_{\nu}^{RJ}}{I_{\nu}^{RJ}} \approx -2y \quad y = \int \left(\frac{kT_{\text{gas}}}{m_{e}c^{2}}\sigma_{T}n_{e}\right)\diff{l}
\end{equation*}
...
\par On the other hand, the surface brightness of the X-ray radiation from the hot ICM can be approximated as
\begin{equation*}
...
\par Assuming that a star at the outer edge of the spiral is on a stable circular orbit around the centre of the galaxy, we can say that (assuming a spherically symmetric mass distribution)
\begin{equation*}
\frac{v_{\text{max}}^{2}}{R} = \frac{GM_{R}}{R^{2}}
\end{equation*}
...
\end{equation*}
Finally, if spiral galaxies have a constant surface brightness $L/R^{2}:=c_{SB}$ (so called \emph{Freeman’s law}), we can take the square of the right and left hand side terms and rewrite it as
\begin{equation}
...
\begin{enumerate}
    \item  Spiegare come si costruisce una scala delle distanze cosmiche. Indicare il numero minimo di indicatori necessari e discuterne il ruolo.
    \item Indicare quale intervallo di redshift è opportuno utilizzare per determinare la costante di Hubble e quale per vincolare i parametri cosmologici, motivando la scelta.
    \item Discutere come la scala delle distanze cosmiche possa essere utilizzata per porre vincoli ai modelli cosmologici. Descrivere il ruolo delle Cefeidi e delle supernovae di tipo Ia, spiegando perché queste ultime sono particolarmente rilevanti come indicatori di distanza.
    \item Discutere la relazione periodo-luminosità delle Cefeidi e come questa può essere calibrata utilizzando indicatori di distanza geometrici.
    \item Discutere due indicatori di distanza geometrici e due indicatori non geometrici.
    \item Discutere la tensione sul valore della costante di Hubble $H_{0}$ confrontando le misure basate sulla scala delle distanze cosmiche con quelle derivate da metodi cosmologici, in particolare CMB e BAO.
    \item  Discutere le tecniche e i principali risultati ottenuti con indicatori di distanza alternativi, quali il lensing gravitazionale di quasar variabili, le fluttuazioni di brillanza superficiale e l’effetto Sunyaev-Zeldovich.
\end{enumerate}