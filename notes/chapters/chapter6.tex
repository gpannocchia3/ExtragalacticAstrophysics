...
A few chapters ago we introduced hte Hubble fork for the morphological classification of galaxies (see Fig.\ref{fgr:hubblefork}). In the present chapter we're going to focus on the fork part of the fork, and so on the so-caled \emph{disk galaxies}. They come in two types:
\begin{itemize}
...
\end{equation}
where we've defined an effective radius $R_{e}$ whitin which is contained half the object's light. The effective radius usually sit in the range $0-5-4\unit{kpc}$.
\par Meanwhile, for the disk is customary to use an exponential model
\begin{equation}
    I(r) = I(0)\exp\left(-\frac{R}{R_{d}}\right)
\end{equation}
which can be converted to magnitude per square arcsecond 
\begin{equation*}
    \mu(r) = \mu(0)+1.086\frac{R}{R_{d}}
\end{equation*}
...
The disk is still rather well-fitted by an exponential distribution of light
\begin{equation}
...
\end{enumerate}
To measure the systemic velocity of a rotating galaxy, we can simply observe the galaxy along its projected minor axis. Under the assumption of an ideal rotating disk, in fact, the rotational contribution to $v_{\text{LOS}}$ vanished along that direction, meaning that $V = v_{\text{sys}}$.
...
\end{equation*}
so that points with fixed $v_{\text{LOS}}$ will have constant $X$, as shown in Fig.\ref{fgr:solidbodyspider}.
\begin{figure}[h!]
    \centering
...
\end{enumerate}
The tilted ring model assums the observed velocity to be decomposable into three different contributions
\begin{equation*}
    V(X,Y) = V_{0}+V_{c}(R)\cos\theta\sin i + V_{r}(R)\sin\theta\sin i
\end{equation*}
where the latter terms is to account for any deviations from circular rotation, including a term $v_{r}$ to model radial motions in the plane.
...
\end{itemize}
As you should know by now, the surface brightness is just
\begin{equation*}
    I = \der{F}{\Omega}
\end{equation*}
...
\end{figure}
\par Where the rotation curve is flat, assuming spherical symmetry for the the dark halo
\begin{align*}
...
\end{equation*}
In the deeep MOND regime, the true acceleration of a test particle on a circular orbit is still the centripetal accelerationl so
\begin{equation*}
...
\begin{equation}
    ...
    \label{eq:MOND_TF}
\end{equation}
In the deep MOND regime, the acceleration would be $a = \sqrt{a_{0}a_{N}}$, where $a_{N}$ is the standard Newtonian acceleration. In other words, in the MOND regime (low accelerations), MOND postulates an acceleration higher than the Newtonian one. Far from the center, where Newton would predict too weak a gravitational pull, MOND would effectively boost the acceleration, so a higher orbital velocity is required to maintain the orbit.
...
\begin{itemize}
    \item The stars in HSB (High Surface Brightness) galaxies experience centripetal accelerations of order $a_{0}$;
...
\end{figure}
\par In MOND, the effective gravitational acceleration does not decrease as rapidly as the Newtonian one $a_{N} = GM/r^{2}$.In the deep-MOND regime,this becomes $a_{N} = a^{2}/a_{0}$, so $a^{2} = a_{0}a_{N} \sim r^{-2}$ rather than $\sim r^{-4}$ as predicted by the Newtonian theory. Hence, The gravitational potential should remain aligned with the gas.
...
We've seen in earlier chapters that the most likely candidate for DM (as of right now) are the WIMPS. But what if DM is instead made of brown dwarfs, planetary bodies, white dwarfs, black holes, neutron stars?
\begin{figure}[h!]
    \centering 
    \includegraphics[width=0.9\textwidth]{img/dmcandidates.png}
\end{figure}
\par Given their nature, MACHOS cannot be observed through RM radiation, but rather by \emph{gravitational micro-lensing}: As a MACHO crosses a background star, the star appears to brighten, then dim. This description, however, could in principle apply to variable stars as well. But there are a few key differences
\begin{itemize}
...
Because the angle of deflection is small, can approximate situation by assuming that the star follows the unperturbed trajectory, as shown in Fig.\ref{fgr:weakencounters}.
\begin{figure}[h!]
    \centering 
...
General relativity predicts that for photons, the bending is exactly twice the Newtonian value, so 
\begin{equation*}
    \alpha = \frac{4Gm}{bc^{2}} = \frac{2R_{s}}{b}
\end{equation*}
where $R_{s}$ is the Schwarzschild radius of a body of mass $m$.
\par Consider sources at distance $d_{s}$ from the observer O, while a point mass lens L is at distance $d_{L}$ from the observer. The observer sees the image I of the source S’ at an angle $\theta$ from the line of sight with respect to the lens. In the absence of deflection, would have measured an angle $\beta$.
...
\par In some cases the lensing is of an image that is so small or faint that one doesn't see the multiple images: The additional light bent towards the observer just means that the source appears brighter.
\par This lensing can have tangible effects in many measurements, as sources which would have otherwise been too dim become visible. This can be helpful, as when one wants to observe objects that would otherwise be too far away.
\par It can also be a problem, for example when we're trying to measure all objects brighter than a certain threshold in a certain region and lensing introduces objects by magnifying objects enough to bring them into the sample.
...
\end{equation}
if all of its matter is composed of microlenses and the sources are at the boundaries of the galaxy ($d_{S}\sim R$)
\begin{equation*}
...
\end{equation*}
Optical depths defined as such agree well with the predictions based on the galactic model of the Galaxy (number of faint stars, brown dawrves, white dawrves, NS/BH and so on). Hence, DM is \emph{not} made by "dark" stars or remnants. So no MACHOS.

\subsubsection*{Timescale for Microlensing Events}

There's no way to determine from a single image whether a given star is being magnified by lensing. We need a series of images to see the star brighten then fade as the alignment changes. It is customary to define a \emph{lensing time scale} which is equal to the physical distance across rhe Einstein ring divided by the relative velocity of the lens
\begin{equation}
...
\begin{equation*}
    \tau_{\text{Lensing}} \approx 40\sqrt{\frac{M}{0.3M_{\odot}}}\unit{d}
\end{equation*}
...
\begin{figure}[h!]
    ...
    \label{fgr:galrot}
\end{figure}
\par As long as we perform our observations within the Solar circle (Ist and IInd quadrants, but technically only the first one can be used), we can build the rotation curve of the MW through the \emph{tangent point method}:
\begin{enumerate}
...
\end{equation*}
We can expand the derivate in the equation for $v_{\text{LOS}}$ and substitute what we've found
\begin{align*}
    & v_{\text{LOS}} = \frac{d}{R_{0}}\sin(l)\cos(l)\left[\frac{V_{0}}{R_{0}}-\left(\der{V}{R}\right)_{R_{0}}\right]\\
...
\end{equation*}
We can Taylor expand once more under the assumption of $d/R_{0}\ll 1$, so that neglecting second order terms (or higher) we obtain 
\begin{equation*}
...
\begin{equation*}
    A = \frac{1}{2}\left[\frac{V_{0}}{R_{0}}-\left(\der{V}{R}\right)_{R_{0}}\right] = \frac{1}{2}\left[\Omega_{0}-\left(\der{(\Omega R)}{R}\right)\right] = 0
\end{equation*}
...
\begin{align*}
    \der{V}{R} &= -(A+B)\\
    \left(\der{V}{R}\right)_{\text{measured}} &= -2.4\pm 1\unit{km}\unit{s}^{-1}\unit{kpc}^{-1}\\
    \left(\der{V}{R}\right)_{\text{Kep}} &= -13.75\unit{s}^{-1}\unit{kpc}^{-1}
\end{align*}
As expected, the Keplerian circular velocity drops $\propto R^{-1/2}$ and therefore the gradient is negative. However, the measured value is much closer to zero: the Milky Way’s rotation curve is flat, i.e. $V(R)\approx \text{const.}$

\section{Spiral Arms}

Let's look once more at Hubble's morphological classification such as that in Fig.\ref{fgr:hubblefork}. Three sub-classes ar usually defined in the Elmegreen classification scheme: \emph{Grand Design} (two well distinguishable spiral arms), \emph{Multiarm} (intermediate, two less prominent large spiral arms and a whole lot of smaller ones) and \emph{Flocculent} (a whole lot of smaller spiral arms).
\par We shall define a \emph{trailing arm} the one whose outer tip points in the direction opposite to galactic rotation, while the outer tip of a \emph{} points in the direction of rotation; in almost all cases the spiral arms are trailing.
\par We also define the \emph{pitch angle} $\psi$ as the angle formed by a spiral arm with the tangent to the galactic disk where spiral and disk intersect. It is simpler understood looking at the equation
\begin{equation}
...
\begin{equation*}
    \cot\psi = \Big|\parder{\phi}{R}\Big| = R\Big|\parder{\Omega}{R}\Big|t = \frac{V}{R}t
\end{equation*}
Using values appropriate for the MW at the Sun position, we'd get
\begin{equation*}
    \psi \approx 2\text{°}\left(\frac{1\unit{Gyr}}{t}\right)
\end{equation*}
Spiral arms structure would be too tightly wound after $\sim 1\unit{Gyr}$ due to differential rotation. Eventually spiral structure would be “smeared out.”
\par This results is much tighter than those observed in Sc galaxies, like the MW, or Sa galaxies: $\psi \sim 5\text{°}$ (Sa) and $10\text{°}\lesssim \psi \lesssim 30\text{°}$ (Sa). The most likely implication is that spiral arms are NOT material features of the galaxy.
...
Let us analyze the orbit of a star in some axisymmetric potential. We also assume that the potential does not depend on time. The star has angular momentum $L_{z} = R^{2}\dot{\phi} = \text{const.}$ and the energy of a star in such potential can be expressed as 
\begin{equation*}
...
\end{equation*}
If we call the second derivatives of the effective potential with respect to $R$ and $z$ respectively $\kappa$ and $\nu$, then the time evolution of $z$ is as follows
\begin{equation*}
    \ddot{z} = -\nu^{2}z
\end{equation*}
...
\begin{equation}
    \kappa^{2}(R_{G}) = \left(\parder{(R\Omega^{2}(R))}{R}+3\Omega^{2}\right)_{(R_{G},0)} = \left(R\parder{\Omega^{2}}{R}+4\Omega^{2}\right)_{(R_{G},')}
\end{equation}
...
\end{equation*}
hence the orbits does not close on itself in an inertial frame (the ratio should have been a rational number) but rather forms a rosette.
...
\par Moreover, we'd also observe that the velocity dispersion along two different directions are different. Suppose to measure the velocities of stars close to us, at $R\approx R_{0}$. Some of these will have guiding centers further out than the Sun, and so are on the
inner parts of their epicycles, while others have their guiding centers at smaller radii. Their speed relative to the Sun will be
\begin{equation*}
...
\end{equation*}
with $m$ are integers. We can then choose a proper $\Omega_{o}$ so that the orbit is closed after $m$ radial oscillations: After a time $mT_{r}$, the star makes a $2\pi$ angle in the rotating frame. We can for example set $m=2$ and take 
\begin{equation*}
    \Omega_{p} = \Omega -\frac{k}{2} \quad k = \frac{2\pi}{T_{r}}
\end{equation*}
So, in the rotating frame at frequency $\Omega_{p}$ orbits are “closed” (ovals). Well, not exactly closed since $\Omega(R)$ and $k(R)$ depend on the radial distance $R$. However, observations suggest that $\Omega - k/2$ is approximately constant. So, there is a frequency $\Omega_{p}$ where all orbits are almost closed.
...
\par However, it shall be noticed that $\Omega_{p}$ is only approximately constant: Since the patter rotational velocity changes as a function of radius (which, in turn, depends on rotation curve), the kinematic density waves also wind up, just more slowly. More importantly this approach isn't exaclty fully self-consistent: The gravity due to the non-axisymmetric nature of the patterns in the disk has to be taken into account, which is usually done by calculating perturbations over an axisymmetric potential\footnote{The interested reader can refer to \cite{1964ApJ...140..646L} for more informations.}.
...
\par There are two cases, then: $\Omega_{p}<\Omega_{*}$ the star moves past the arms; $\Omega_{p}>\Omega_{*}$ the arms move past the stars. Stars respond so as to strengthen the spiral only if the perturbing frequency $m|\Omega_{p}-\Omega_{*}|$ is slower than the epicyclic frequency $\kappa(R)$ at that radius.
...
\end{itemize}
Note, however, that bars \emph{are not} density waves, but material structures rotating as rigid bodies. Like spirals, the figure of the bar is not static, but rotates with some pattern speed $\Omega_{p}$. But, unlike spiral arms, bars are not density waves; most of their stars always remain within the bar. Inside a rotating bar, stars and gas no longer follow near-circular paths, but stay close to orbits that close on themselves as seen by an observer moving with the bar’s rotation.
