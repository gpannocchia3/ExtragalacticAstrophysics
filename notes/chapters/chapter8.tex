...
\end{equation}
where we're assuming each star to undergo its entire mass loss istantaneously at te end of a well-defined lifetime, usually the main sequence. In (\ref{eq:massejectionrate}) we've defined $m_{\text{TO}} = m_{t=\tau_{TO}}$ as the mass at turn-off, $m_{\text{UP}}$ as the IMF upper mass cut-off, $m$ the stellar mass and $w$ the mass of the remnant.
\par At a time $t$ the ejected mass is due to the stars born at $t-\tau(m)$, that died at time $t$. At the epoch $t-\tau(m)$ the star formation rate was $\Psi(t-\tau(m))$ and the IMF $\Phi(m,t-\tau(m))$.
\par Once a star reaches its endpoint (after a time $\tau(m)$), we assume that a remnant is left behind: A white dwarf for low and intermediate mass stars ($m \lesssim 8-10M_{\odot}$) and a neutron star or black hole for more massive stars. This can be quantified by defining a remnant mass ($w$) that remains locked away at later times. The remnants production rate is then defined as
\begin{equation}
...
\end{align*}
It is useful to define the \emph{returned fraction} $R$, that is is the fraction of mass that, after forming stars at time $t$, is returned to the ISM
\begin{equation}
...
\end{equation*}
but since $Z\ll 1$ in all situatins of interest, we can write 
\begin{equation}
...
\end{equation}
It is possible to show that, whenn the metallicity of either the gas or the young stars skyrockets to infinity ($\mu \to 0$), the mean stellar abundance tends to the net yield $p$.
...
\begin{equation*}
    \frac{M_{*}(Z<\beta Z_{1})}{M_{*}(Z<\beta Z_{1}} = \frac{1-\mu^{\beta}}{1-\mu}
\end{equation*}
...
\end{equation*}
Under IRA and assuming gas expelled has same metallicity as the ISM
\begin{align*}
    &M_{g}\der{Z}{t} = \Psi(t)p\alpha\\
...
\par The leaky box model can explain the metallicity distribution of the halo. Where did all of that gas lost by the halo end up? The mass of bulge is close to estimated mass lost from halo; on top of that, the angular momentum distribution of the halo and bulge are more similar than are the halo and disk populations.
\par Thes points suggest that the bulge, not the disk, may have received most of the relatively low angular momentum gas from the halo and/or halo object. The halo+disk system wouldn't form a "closed box", but the halo+bulge one would.
\begin{figure}[h!]
    \centering 
...
\begin{figure}[h!]
    ...
    \label{fgr:snyields}
\end{figure}
\par Consider SN rates from short-duration burst of star formation. The Type II SNe rate is proportional to the standard SFR, while Type Ia SNe follow after a delay of $\sim 10^{8}-10^{9}\unit{yr}$ (a broad distribution of precursor lifetimes). If the burst is short, only Mg-rich\footnote{Magnesium is just an example. You could have chosen any other element that is produced by $\alpha$-addiction in the late evolutionary stages of stars and our point would still be holding.} SNII products are incorporated into the stars, while SNIa ejecta just escape from the galaxy. This leads to an higher than usual [Mg/Fe].
...
\begin{figure}[h!]
    \centering 
    \includegraphics[width=0.9\textwidth]{img/irafailures.png}
