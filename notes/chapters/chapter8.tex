...
\begin{equation*}
    M_{*}(Z<Z_{1}) = M_{g}(0)-M_{g}(Z_{1}) = M_{g}(0)\left[1-\exp\left(-\frac{Z_{1}}{p}\right)\right]
\end{equation*}
therefore the fraction of stars at time $t$ with $Z<\beta Z_{1}$ is given by
\begin{equation*}
    \frac{M_{*}(Z<\beta Z_{1})}{M_{*}(Z<Z_{1})} = \frac{1-\mu^{\beta}}{1-\mu}
\end{equation*}
as follows from (\ref{eq:mu_def}).
\par You should also keep in mind that in a closed-box model, at any given epoch, stars cannot have a metallicity higher than the current gas metallicity, because each stellar generation inherits the gas metallicity at birth and the gas metallicity increases monotonically with time.
...
\begin{itemize}
    \item The IMF changes with time (through metallicity): For example, if we had bimodal star formation where the early, metal-poor IMF only made higher mass stars, then the G-dwarf problem could be solved within a closed-box context;
...
\end{equation*}
Under IRA and assuming the gas expelled has the same metallicity as the ISM in (\ref{eq:mu_def})
\begin{align*}
    &M_{g}\der{Z}{t} = \Psi(t)p\alpha\\
...
\begin{equation*}
    \ln\left(\frac{Z_{in}-Z_{0}+p}{Z_{in}-Z+p}\right) = \frac{M_{t}-M_{t}(0)}{M_{g}}
\end{equation*}
We can define $\nu = (M_{t}-M_{t}(0))/M_{g}$ so that the last equation becomes
\begin{equation*}
...
\begin{align*}
    Z = p(1-e^{-\nu}) &= p\left[1-\exp\left(-\frac{M_{t}-M_{t}(0)}{M_{0}}\right)\right]\\
    &= p\left[1-\exp\left(-\frac{M_{t}-M_{g}}{M_{g}}\right)\right]
\end{align*}
