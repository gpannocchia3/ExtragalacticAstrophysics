...
\par Because the density of the ISM is very low, the particles have a large mean free path. As the typical cross section and mean free path of an atom for collisions is of the order
\begin{equation*}
...
\par CO is the next most abundant molecule and has bright lines at microwave frequencies. It can be used to trace cold Hydrgoen gas, but relies on a conversion factor from CO to $\text{H}_{2}$ mass, and has been detected in host galaxies of some of the most distant QSOs.
...
The Orion Nebula is a typical H$_{\text{II}}$ region, associated with a nearby molecular cloud. H$_{\text{II}}$ regions are dominated by the warm interstellar medium but also have cool and hot components. Part of the dense gas of the molecular cloud has collapsed and formed stars, among which only the more massive ones (O stars) can ionize Hydrogen and heavier elements like Oxygen, Nitrogen, Sulfur etc.
\par What is the energy source that makes possible such large ionized clouds? Most likely, it is the energy delivered by supernovae. After about 200 years the ISM temperature drops to $10^{6}-10^{7}\unit{K}$. At these temperatures, cooling is mostly achieved through thermal Bremsstrahlung, a rather inefficient process. A typical example of a supernova remnant is the Crab nebula. It is about 1000 years old and still expands with a velocity of about $1100\unit{km}\unit{s}^{-1}$.
...
Detailed nebular structure depends on density distribution of surrounding gas, so for the moment we'll consider an ideal configuration, that of a star in a uniform medium of pure Hydrogen. We'll assume that:
\begin{enumerate}
...
\par To expand the H$_{\text{II}}$ region by $\text{d}R$, the star must ionize $4\pi R^{2}n_{\text{H}}\text{d}R$ atoms. In the interior of the region, with $n_{i}$ and $n_{e}$ the number densities of ions and electrons, and $\alpha(T)$ the recombination rate coefficient (weakly dependent on temperature) we have
\begin{equation*}
...
\par As the region grows, it needs more photons to balance recombinations. When an equilibrium us reached, you get the size of region by setting $\dot{R} = 0$, and thus
\begin{equation*}
...
\end{equation*}
In the limit of a strong shock, condition often met in astrophysical shocks, $\mathcal{M}_{1} \gg 1$\footnote{Here $\mathcal{M}_{1}$ is the upstream Mach number.}, we find out that 
\begin{equation*}
...
\end{align*}
If you recall that the collisional coefficients are related through the Boltzmann factor
\begin{equation*}
...
\par Below the critical density, the line emissivity is independent of $A_{21}$ and is proportional to the collisional excitation rate
\begin{equation*}
...
\begin{enumerate}
    ...
    \item Continuum IR emission $\to$ diffuse galactic emission correlated with H$_{\text{I}}$ and CO, young and old stars with large infrared excesses;
...
\end{equation*}
where $Q_{\text{ext}}(\lambda)$ is the efficiency factor for extinction. The optical depth along the line sight with volumetric dust density $n_{d}$ is then
\begin{equation*}
...
\par The mathematical basis is a multipole expansion of the scattered wave in terms of (vector) spherical harmonics, each multiplied by a radial Bessel function, plus the application of appropriate boundary conditions at the surface of the sphere. The basic parameter, that measures radius in terms of wavelength is $x = 2\pi a/\lambda$, defines three regimes:
\begin{enumerate}
...
\end{equation*}
then equating the expressions for $m_{\text{obs}}$ with and without extinction
\begin{equation*}
    5\log r_{\text{true}} + A(r) = 5\log r_{\text{inf}}
\end{equation*}
which finally yields
\begin{equation*}
    r_{\text{inf}} = 10^{A(r)/5}r_{\text{true}}
\end{equation*}
...
\begin{equation}
    \text{E}(\lambda_{1} - \lambda_{2}) = A(\lambda_{1})-A(\lambda_{2})
    \label{eq:reddending}
\end{equation}
\begin{figure}[h!]
    \centering
...
\par The most reliable and widely used technique involves the "pairing" of stars of identical spectral type (so about the same temperature) and luminosity class (so about the same surface gravity) but unequal extinction
\begin{align*}
...
\end{equation}
Let’s repeat the observations of the two stars in two filters $\lambda$ and $\lambda_{2}$, with extinction at $\lambda_{2}$ almost negligible, for example the k-band. This way we can calculate
\begin{equation*}
...
In our Galaxy $\text{E}(B-V)$ is found to be directly proportional to the column density of Hydrogen in all its forms $N(\text{H}_{\text{tot}})$. If you recall that
\begin{equation*}
    \text{E}(B-V) \propto A_{V} \propto \sigma_{V}N(X)
\end{equation*}
then this claim implies that 
\begin{equation}
...
\begin{equation*}
    \tau_{\lambda} = \langle \frac{\rho_{d}}{\rho_{\text{gas}}}\rangle \langle \frac{m_{\text{gas}}}{m_{d}}\rangle Q_{\lambda}\pi a^{2}N_{\text{H}}
\end{equation*}
...
\end{equation*}
Numerically comparing with the empirical value $A(V)$ we've given somewhere in the above produces an estimate of the dust-to-gas ratio
\begin{equation*}
...
Studying emission maps of dust, we find out that it glows particularly bright in the FIR (Far Infrared), so it will be our goal to correlate the reddening to the FIR flux. The most commonly used dust maps today are those created by \cite{1998ApJ...500..525S} from the COBE FIR maps.
...
\par The dust embedded in a H$_{\text{II}}$ region is exposed to an intense radiation field, so for grains at a distance $d$ from a point source of luminosity $L$ we have 
\begin{equation*}
...
\par They are the same, as follows from Kirchoff's law: Good (bad) absorbers are good (bad) emitters\footnote{For a more rigurous definition you can see either \cite{choudhuri} or \cite{1986rpa..book.....R}.}. The rate of heating of the grain through absorption of radiation can be written
\begin{equation*}
    \left(\der{E}{t}\right)_{\text{abs}} = \int \frac{u_{\nu}}{h\nu} \cdot c \cdot h\nu \cdot Q_{\text{abs}}(\nu)\pi a^{2} \diff{\nu}
\end{equation*}
where we can use the spectrum-averaged absorption cross section we've defined a few lines ago. The radiative heating rate is simply 
\begin{equation*}
...
\end{equation*}
with $\langle Q_{\text{em}} \rangle_{T_{d}}$ the Planck-averaged emission efficiency. At equilibrium the absorption and emission rate must be equal, thus 
\begin{equation*}
...
\end{equation*}
\par Dust grains are good absorbers in the UV, so we can approximate $$\langle Q_{\text{abs}} \rangle_{*} \approx \langle Q_{\text{abs}}(UV) \rangle_{*}$$ Also, dust can't emit at $\nu >$ NIR, so we can approximate $$\langle Q_{\text{abs}} \rangle_{T_{d}}\approx \langle Q_{\text{abs}}(IR) \rangle_{T_{d}}$$ In conclusion
\begin{equation*}
...
\par Please note that we have used the absorption component $Q_{\text{abs}}$, not the scattered one (which is not heating the grain). Using the modified equilibrium, interstellar dust is warmed:
\begin{itemize}
    \item By a weak isotropic radiation field. This cold component is called
interstellar "cirrus";
    \item In H$_{\text{II}}$ regions;
    \item Deep inside GMCs, where it's shielded from all external radiation;
    \item In AGNs, where dust can be appreciably warmer.
\end{itemize}
...
\end{itemize}
there are just as many environments that are hostile to the formation of grains:
\begin{itemize}
...
\begin{enumerate}
    \item Discutere gli effetti della polvere interstellare sulla radiazione elettromagnetica, con particolare riferimento ad assorbimento, scattering ed emissione termica.
    \item Definire che cos’è una curva di estinzione. Descrivere le principali caratteristiche della curva di estinzione della Via Lattea e interpretarle nel quadro della teoria di Mie.
    \item Discutere se la curva di estinzione interstellare abbia una forma universale. Motivare la risposta considerando la dipendenza dall’ambiente e dalle proprietà dei grani di polvere. Descrivere come viene parametrizzata la curva di estinzione nella Via Lattea.
    \item Indicare il valore tipico del rapporto in massa tra polveri e gas nella nostra Galassia e descrivere i principali metodi osservativi per stimarlo. Spiegare perché, in una galassia come la Via Lattea, la luminosità infrarossa emessa dalla polvere può essere confrontabile, come ordine di grandezza, con la luminosità stellare emessa nell’UV, ottico e vicino infrarosso.
    \item Discutere il meccanismo di emissione della polvere interstellare e fare degli esempi.
    \item Spiegare perché il mezzo interstellare presenta una struttura multifase. Discutere i principali meccanismi di riscaldamento e raffreddamento che determinano l’equilibrio termico delle diverse fasi.
    \item Discutere gli effetti di una stella massiccia sul mezzo interstellare circostante, con particolare riferimento alla formazione ed evoluzione delle regioni H$_{\text{II}}$.
    \item Discutere il concetto di shock isotermo o adiabatico sul mezzo interstellare. Fornire alcuni esempi astrofisici rilevanti.
    \item Discutere i principali meccanismi di raffreddamento del mezzo interstellare in funzione della temperatura. Spiegare perché le righe proibite di ioni metallici svolgono un ruolo particolarmente importante nel raffreddamento del mezzo interstellare.
\end{enumerate}